...
void GameState::Update(float deltaTime) 
{
	...
	if (input->IsMousePressed(MouseButton::LBUTTON))
	{
		...
		mBodyAnchor.SetVelosity(mCamera.GetDirection() * 20.0f);
	}
	Math::Vector3 velocity = mBodyAnchor.GetVelocity();
	...
	mBodyAnchor.SetVelosity(velocity);
}
void GameState::Render() 
{
	//Start at 1 because 0 is managed by physics
	// also starting at 1 means that previus value is accessible 
