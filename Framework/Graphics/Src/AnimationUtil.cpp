...
namespace
{
	void ComputeBoneTransformsReursive(const Bone* bone, AnimationUtil::BoneTransforms& boneTransforms)
	{
		if (bone != nullptr)
		{
			...
			{
				ComputeBoneTransformsReursive(child, boneTransforms);
			}
		}
...
void AnimationUtil:: ComputeBoneTransforms(ModelId modelId, BoneTransforms& boneTransforms)
{
	...
	if (model != nullptr && model->skeleton != nullptr)
	{
		...
		ComputeBoneTransformsReursive(model->skeleton->root, boneTransforms);
