...
{
	ASSERT(blockSize > 0, "BlockAllocator: %s invalid blocksize", mName.c_str());
	...
	LOG("%s allocated %zu block, blocks size",mName.c_str(), capacity, blockSize);
}
BlockAllocator::~BlockAllocator() 
{
	...
	std::free(mData);
	mData = nullptr;
	LOG("%s detructed, Allocated: %zu, Freed : %zu",
		mName.c_str(), mBlocksAllocatedCurrent, mBlocksFreed, mBlockHighest);
}
void* BlockAllocator::Allocate()
{
	...
	mBlockHighest = std::max(mBlockHighest, mBlocksAllocatedCurrent);

	LOG("%s allocated blookcs at %p, Allocated: %zu, Highest: %zu",
		mName.c_str(), freeBlock, mBlocksAllocatedCurrent, mBlockHighest);
