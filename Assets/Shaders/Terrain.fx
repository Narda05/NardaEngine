...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
    ...
    //emissive
    float4 emissisve = materialEmissive;
    ...
    float4 finalColor = (materialEmissive + ambient + diffuse) * diffuseColor + specular;
    if (useShadowMap)
    {
        ...
        if (saturate(u) == u && saturate(v) == v)
        {
            ...
            {
                finalColor = (materialEmissive + ambient) * diffuseColor;
            }
