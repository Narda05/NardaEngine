// Simple shader that aplies post prossessing effects to a scene 
...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
    ...
    else if (mode == 5) //combine 2 textures
    {
        ...
        color1.a *= param0; // set alpha from param0
        finalColor = (color0 * (1.0f - color1.a)) + (color1 * color1.a); // no alpha 
    }
    ...
    {
        float waveValue = input.texCoord.x * (3.121592f * param1);
        float2 textCoord = input.texCoord;
        textCoord.y += sin(waveValue) * param0;
        finalColor = textureMap0.Sample(textureSampler, textCoord);
    }
        return finalColor;
}